...
public class minPlayer extends Player {
	/*
	 * TODO: find the best value... i guess sth around 25
	 * how many fields must be free to change to offensive strategy?
	 * as soon as less than changeToOffensiveStrategyThreshold fields are free, minPlayer gets offensively
	 * 
	 * if it's 25, and MaxPlayerWithStragey : MinPlayer (Max begins) => Max wins :-(
	 */
	private int changeToOffensiveStrategyThreshold = 15;
	...
	public Coordinates nextMove(GameBoard gb) {
		...
		bestCoord = coordsWithBestNumOfChangeOfArray(checkCoords, gb);
		if (bestCoord != null) {
			return bestCoord;
		}
	...
	/**
	 * Checks, if playing testCoord will change less slices than previousNumOfChanges
	 ...
	 * @return returns the minimum value of fields which will be changed. <br>
	 * 			returns 0 if testCoord is not a valid move <br>
	 * 			returns a negative number if testCoord would change more fields (i.e.) fields changed by testCoord * (-1) = return value<br>
	 * 			returns a positive number if testCoord would be a better (or equal) move 
	 */
	int smallerNumOfChanges(int previousNumOfChanges, Coordinates testCoord, GameBoard gb) {
		...
		if (checkMove(testCoord, gb)) {
			...
			/*
			 * if it isn't a valid move, just return 0
			 */
			return 0;
		}
	}
	...
	/**
	 * Looks for the best move (which will change the least fields) out of the array of coordinates
	 ...
	 */
	Coordinates coordsWithSmallestNumOfChangeOfArray( int coords[][], GameBoard gb ) {
		Coordinates bestCoord = null;
		...
		for( int i=0; i<coords.length; i++ ) {
			...
			if (checkMove(testCoord, gb) ) {
				numOfChanges = smallerNumOfChanges(bestNumOfChanges, testCoord, gb);
				
				/*
				 * if bestNumOfChanges is still == 0, then take the tested coordinates as better ones ^^
				 * if numOfChanges > 0, smallerNumOfChanges has found a better or equal coordinate
				...
				if (bestNumOfChanges == 0  ||  ( (numOfChanges > 0) && (numOfChanges != bestNumOfChanges) )) {
					 ...
					 * => save all "bestCoords" into an array, and select from this array after all checks have passed
					 */
				}
			}
		}
	...
	/**
	 * 
	 * TODO: play also offensively, if i have only few points left 
	 * @param coords
	 * @param gb
	 * @return
	 */
	Coordinates coordsWithBestNumOfChangeOfArray( int coords[][], GameBoard gb ) {
		if (getFreeFields(gb) <= changeToOffensiveStrategyThreshold) {
			System.out.print("o");
			return coordsWithGreatestNumOfChangeOfArray(coords, gb);
		} else {
			return coordsWithSmallestNumOfChangeOfArray(coords, gb);
		}
	}
